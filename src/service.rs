...
use crate::i18n::I18n;
use crate::models::{AppConfig, InternalUser, ItemType, Library, LibraryItem};
...
impl<C: AbsClient + ?Sized> LibraryService<C> {
    ...
    pub async fn get_filtered_items(
        ...
        let mut parsed_items: Vec<LibraryItem> = items_data
            .results
            .into_par_iter()
            .filter_map(|item| {
                    ...
                    Some(LibraryItem {
                        ...
                        series: item
                            ...
                            .map(|s| {
                                ...
                                let re = SERIES_CLEANUP_RE
                                    .get_or_init(|| regex::Regex::new(r"#.*$").unwrap());
                                s.split(',')
                                    .map(|n| n.trim().replace(re.as_str(), "").trim().to_string())
                                    .collect()
                            })
                            .unwrap_or_default(),
                        format,
            ...
            parsed_items = parsed_items
                ...
                .filter(|item| {
                    if type_query == Some(&ItemType::Authors) {
                        ...
                        if let Some(re) = &name_query_re {
                            return item.genres.iter().any(|g| re.is_match(g))
        ...
        if let Some(author) = &query.author {
            let re = regex::RegexBuilder::new(&regex::escape(author))
                .case_insensitive(true)
                .build()?;
            parsed_items = parsed_items
                .into_par_iter()
                .filter(|item| item.authors.iter().any(|a| re.is_match(&a.name)))
                .collect();
        }

        if let Some(title) = &query.title {
            let re = regex::RegexBuilder::new(&regex::escape(title))
                .case_insensitive(true)
                .build()?;
            parsed_items = parsed_items
                .into_par_iter()
                .filter(|item| {
                    item.title.as_deref().is_some_and(|t| re.is_match(t))
                        || item.subtitle.as_deref().is_some_and(|t| re.is_match(t))
                })
                .collect();
        }
    ...
    pub async fn get_categories(
        ...
        let distinct_type: HashSet<String> = items_data
            .results
            .into_par_iter()
                ...
                match type_ {
                    "authors" => {
                        if let Some(names) = item.media.metadata.author_name {
                            Self::insert_split_names(&mut acc, &names);
                        }
                    }
                    "narrators" => {
                    ...
                    "genres" => {
                        if let Some(genres) = item.media.metadata.genres {
                            Self::insert_vec_names(&mut acc, &genres);
                        }
                        if let Some(tags) = item.media.metadata.tags {
                            Self::insert_vec_names(&mut acc, &tags);
                        }
                    }
                    "series" => {
                        if let Some(series) = item.media.metadata.series_name {
                            Self::insert_split_names(&mut acc, &series);
                        }
                    }
                    _ => {}
                }
        ...
        if query.start.is_none() && self.config.show_char_cards {
            ...
            for item in &distinct_type_array {
                let normalized = Self::normalize_start_char(item);
                let key = if normalized.as_str() >= "A" && normalized.as_str() <= "Z" {
                    normalized
                } else {
                    String::new()
                };
            ...
            if let Some(start) = &query.start {
                distinct_type_array.retain(|item| {
                    let normalized = Self::normalize_start_char(item).to_lowercase();
                    normalized == *start
                });
            }
    ...
    fn insert_split_names(acc: &mut HashSet<String>, names: &str) {
        for name in names.split(',') {
            acc.insert(name.trim().to_string());
        }
    }

    fn insert_vec_names(acc: &mut HashSet<String>, names: &[String]) {
        for name in names {
            acc.insert(name.trim().to_string());
        }
    }

    fn normalize_start_char(item: &str) -> String {
        let start_char = item
            .chars()
            .next()
            .unwrap_or(' ')
