...
impl<C: AbsClient + ?Sized> LibraryService<C> {
    ...
    pub async fn get_filtered_items(
        ...
        if query.q.is_some() || query.type_.is_some() {
            ...
            let re = utils::create_regex_or_empty(search_term);

            let type_query = query.type_.as_ref();
            let name_query_re = query
                .name
                .as_deref()
                .map(|n| utils::create_regex_or_empty(n));
        ...
        if let Some(author) = &query.author {
            let re = utils::create_regex(author)?;
            parsed_items = parsed_items
                .into_par_iter()
        ...
        if let Some(title) = &query.title {
            let re = utils::create_regex(title)?;
            parsed_items = parsed_items
                .into_par_iter()
                .filter(|item| {
                ...
                match type_ {
                    ...
                    "genres" => {
                        if let Some(genres) = item.media.metadata.genres {
                            for g in genres {
                                acc.insert(g.trim().to_string());
                            }
                        }
                        if let Some(tags) = item.media.metadata.tags {
                            for t in tags {
                                acc.insert(t.trim().to_string());
                            }
                        }
                    }
                    "series" => {
