...
fn bench_service_layer(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        let items_response = AbsItemsResponse {
            results: items.clone(),
        };
        ...
        group.bench_with_input(
            BenchmarkId::new("get_filtered_items", n_items),
            &n_items,
            |b, &_| {
        ...
        group.bench_with_input(
            BenchmarkId::new("get_categories_authors", n_items),
            &n_items,
            |b, &_| {
                b.to_async(&rt).iter(|| async {
                    service
...
fn bench_handlers(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("handler_get_library", n_items),
            &n_items,
            |b, &_| {
                b.to_async(&rt).iter(|| async {
...
fn bench_xml_layer(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        let library_items: Vec<abs_opds::models::LibraryItem> = abs_items
            .into_iter()
            .map(|i| abs_opds::models::LibraryItem {
                id: i.id,
        ...
        let user = mock_user();
        let lib = abs_opds::models::Library {
            id: "lib1".to_string(),
            name: "Lib".to_string(),
            icon: None,
        };
        ...
        group.bench_with_input(
            BenchmarkId::new("xml_build_entries", n_items),
            &n_items,
            |b, &_| {
                b.iter(|| {
